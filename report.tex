...
\end{itemize}   
and participants are expected to classify comments to any subset of these classes (so the problem is a multi class, multi label classification). Dataset that's provided for the challenge is from Wikipedia's talk page edits and goal of the challenge is to help online discussion to become more productive and respectful. 
...
Looking at the observations of the comments, it seems that comments that toxic comments could also be identified by things that are normally processed out. (e.g. all caps or multiple explanation marks). However as it was also observed, classes can be also inferred from just words they contain which is why we fully pre process the data. 
...
As it was observed in the data analysis, a simple method with doc2vec or bag of words should be enough for the task. For the classification, we chose to use methods presented in table \ref{table:methods}. 
...
\textbf{BOW classifier}

For the first model we used neural network with BOW-vectors. As a input, we take a one hot vector that is connected by 2 hidden layer to a output layer of 6-sigmoids. With this implementation we found that memory required to complete the code is quite significant and we had to run the model on computers of the computer lab.

\vline

\noindent \textbf{DOV2VEC with gensim}

For doing DOC2VEC we train the embedding with gensim and use the vectors as a input to the neural network. \textbf{TODO: What kind of Neural Netwokrs use the embedding!} (Normal NN, LSTM)

\section{Experimental results}

We obtained results as seen in table \ref{table:results}.
...
In the table we can see that the neural network with BOW and the sklearn's methods (that also utilize BOW-presentation) are dooing the best while predicting the output.

\section{Conclusions}

As seen on the previous chapter, the sklearns simple methods do really well on the problem which might be caused by that the toxic comments have explicit words that are easy to find, even with these simple models. We also achieved results that were good using neural networks.

We expect that one could achieve even better results utilizing the capital letter and special symbols in the data that are specific to some inputs and this could be one thing to be improved. Another thing that the work could be continued on is making the LSTM work. 
